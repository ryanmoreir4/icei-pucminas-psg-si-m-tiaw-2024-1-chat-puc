...
async function adicionaResposta() {
  ...
  let strPalavrasChave = document
    .getElementById("campo-PalavrasChave")
    .value.trim();
...
document
  .getElementById("btn-inserir")
  .addEventListener("click", adicionaResposta);
...
async function verificaPalavrasChaveDuplicadas(palavrasChave) {
  ...
  return palavrasChave.some((palavra) =>
    objDados.Respostas.some((item) => item.palavras_chave.includes(palavra))
  );
}
...
document
  ...
  .addEventListener("click", removeRespostasSelecionadas);
...
async function imprimeRespostas() {
  ...
  objDados.Respostas.forEach((resposta, i) => {
    strHtml += `<tr>
        <td><input type="checkbox" id="select-resposta" data-id="${
          resposta.id
        }"></td>
...
document.getElementById("btn-alterar").addEventListener("click", async () => {
  ...
  const novasPalavrasChave = document
    .getElementById("campo-PalavrasChave")
    .value.trim()
    .split(",")
    .map((p) => p.trim());
  ...
  const response = await fetch(
    `http://localhost:3000/respostas/${respostaId}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
