$(document).ready(function() {
    ...
    $('#registration-form').submit(function(event) {
        ...
        if (!verificarCheckboxes()) {
            $('.header').after('<span class="error">Por favor, selecione pelo menos um papel.</span>');
            isValid = false;
        }

        if (password !== confirmPassword) {
            $('#confirm-password').after('<span class="error">As senhas não coincidem.</span>');
            isValid = false;
        }
        ...
        if (isValid) {
            ...
            const novoUsuario = { fullName, age, contactNumber, city, email, password, roles };
